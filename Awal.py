...
def logo():
	os.system("clear")
	print(f"{biru}SV NO WHATSAP GUE OY {W}AWAL NURDIANTO{biru} ! ! !{G}")
	os.system("xdg-open https://wa.wizard.id/c7b917")
	time.sleep(10)
	os.system("clear")
	print (f"""{ungu}	|
{ungu}       ( )
...
{ungu}     |'._.'|   {biru}╚═╝{W}┴└─└─┘ ┴ ┴ ┴┴─┘ {biru}╚═╝{W}╚═╝╚╩╝
{ungu}     | {Y}1{W}.{Y}7{ungu} |{W}   [{Y}•{W}] Info sc{R} :{W} Spam Wa {R}({biru}sms,call,wa{R})
{ungu}   .'|  |  |`. {W}[{Y}•{W}] Info Pembuat Scrip{R} :{W} AWAL NURDIANTO
{ungu}  (  |  |  |  )
{ungu}  |,-'--|--'-.|{R}   ({W} Version Script : 1.7 SCRIP BY AWAL N.D )
{abu} ------------------------------------------------------
	 {W}[{Y}•{W}] Ketik{R}:{G} CTRL {R}+{G}z {W}Untuk Berhenti
	 {W}[{Y}•{W}] DIBUAT OLEH          : AWAL NURDIANTO
	 {W}[{Y}•{W}] NAMA INSTAGRAM : awalnurdianto10
""")
